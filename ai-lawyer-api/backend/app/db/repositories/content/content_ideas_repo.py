...
def _build_filters(
    ...
    if title:
        ...
        clauses.append(f"title ILIKE ${len(values)}")
    if persona:
        values.append(f"%{persona}%")
        clauses.append(f"persona ILIKE ${len(values)}")
    if keywords:
        values.append(f"%{keywords}%")
        clauses.append(f"keywords ILIKE ${len(values)}")
...
async def count(
    ...
    *,
    ...
    clauses, values = _build_filters(title, persona, keywords)
